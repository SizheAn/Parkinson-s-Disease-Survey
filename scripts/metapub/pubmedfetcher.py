...
class PubMedFetcher(Borg): 
    ...
    def _eutils_article_by_pmid(self, pmid):
        pmid = str(pmid)
        try:
            result = self.qs.efetch(args={'db': 'pmc', 'id': pmid})
    ...
    def _eutils_article_by_pmcid(self, pmcid):
        ...
        return self._eutils_article_by_pmid(pmcid)
    ...
    def _eutils_pmids_for_query(self, query='', since=None, until=None, retstart=0, retmax=250,
        ...
        created_date_template = '"%s"[PubDate]'
       # date_range_template = ' ("open access"[filter] AND %s : %s)'
        ...
        result = self.qs.esearch({ 'db': 'pmc', 'term': query,
